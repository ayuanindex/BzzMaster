...
import com.shenkong.bzzmaster.net.exception.ApiException;
import com.shenkong.bzzmaster.net.exception.CustomException;
...
public class ResponseTransformer<T> {
    ...
    private static class ResponseFunction<T> implements Function<Response<T>, ObservableSource<T>> {
        ...
        @Override
        public ObservableSource<T> apply(Response<T> resultBean) throws Exception {
            int code = resultBean.code();
            String message = resultBean.message();
            if (code == 200) {
                return Observable.just(resultBean.body());
            } else {
